...
typedef struct TString
{
    ...
    vm_value3<unsigned int> hash;
    vm_value4<unsigned int> len;
...
typedef struct Udata
{
    ...
    vm_value4<struct Table*> metatable;
...
typedef struct Proto
{
    ...
    vm_shuffle8(vm_shuffle_semicolon, vm_value2<TValue*> k,              // constants used by the function
    vm_value2<Instruction*> code,      // function bytecode
    vm_value2<struct Proto**> p,       // functions defined inside the function
    vm_value2<uint8_t*> lineinfo,      // for each instruction, line number as a delta from baseline
    vm_value2<int*> abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
    vm_value2<struct LocVar*> locvars, // information about local variables
    vm_value2<TString**> upvalues,     // upvalue names
    vm_value2<TString*> source);

    vm_value3<TString*> debugname;
    vm_value4<uint8_t*> debuginsn; // a copy of code[] array with just opcodes
    ...
    vm_shuffle9(vm_shuffle_semicolon, int sizecode,
    int sizep,
    int sizelocvars,
    int sizeupvalues,
    int sizek,
    ...
    vm_shuffle4(vm_shuffle_semicolon, uint8_t nups, // number of upvalues
    uint8_t numparams,
    uint8_t is_vararg,
...
typedef struct Closure
{
    ...
    union
    {
        struct
        {
            vm_value4<lua_CFunction> f;
            vm_value2<lua_Continuation> cont;
            vm_value1<const char*> debugname;
            TValue upvals[1];
        } c;

        struct
        {
            vm_value4<struct Proto*> p;
            TValue uprefs[1];
        } l;
    };
} Closure;
...
typedef struct Table
{
    ...
    vm_shuffle5(vm_shuffle_semicolon, uint8_t tmcache,    // 1<<p means tagmethod(p) is not present
    ...
    vm_shuffle4(vm_shuffle_semicolon, vm_value4<struct Table*> metatable,
    vm_value4<TValue*> array,  // array part
    vm_value4<LuaNode*> node,
    GCObject* gclist);
} Table;
// clang-format on
...
#define luaO_nilobject ((TValue*)luao_nilobject_addy)
